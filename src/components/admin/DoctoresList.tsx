    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-800 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="bg-blue-500 dark:bg-blue-600 text-white dark:text-gray-100 px-3 py-1 rounded-full text-sm font-semibold">
            {filteredDoctores.length} {t('doctores.title').toLowerCase()}
          </div>
        </div>
      </div>
      ...
      <div className="p-6 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={t('doctores.searchPlaceholder')}
            value={searchTerm}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {filteredDoctores.map((doctor, index) => (
              <tr key={doctor.id} className={`hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors duration-200 ${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700'}`}>
      ...
      {filteredDoctores.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">{t('doctores.noDoctores')}</h3>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            {searchTerm ? t('doctores.noDoctoresMessage') : t('doctores.noDoctoresMessage')}
          </p>
        </div>
      )}
    </div>
